...
We continued tidying our data set. Further on we made new variables to filter and explore the datasett. For instance PVol_High as categorical varible ( \>/\< 100 ml) The variables unit_norm, ttr_num, TimeToRecurrence_days_new, were all made in the purpose of making the variable TimeToRecurrence_weeks_new, for calculations with the same numeric category (weeks). We met a little rocky road making this new variable, therefor keeping some duplicates to remember the steps of the string.
...
Once we looked over the variables we then realized that we wanted tidy the variable Hosp, deleting characteters and keeping the numer (1-3) of the treathing hospital.

```{r}

joint_exam_data <- read_delim(here("data", "joint_exam_data_2025-09-09.txt"),
                             delim = "\t")
joint_exam_data <- joint_exam_data %>% mutate(Hosp = gsub("Hosp", "", Hosp)) %>% rename("Hospital" = "Hosp")
...
1)  Stratify your data by a categorical column and report min, max, mean and sd of a numeric column for a defined set of observations
...
After the latest tidying we and the stratifying work, we saved the data sett with the new name ggplot_exam_data, as getting ready for task 4 concerning ggplot.
...
Finally, we updated the codebook.
