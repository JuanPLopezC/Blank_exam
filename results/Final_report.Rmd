...
We had different tasks to develop based on these databases and the new skills learned from R. In the following sections, we describe our work to fulfill these tasks. We hope you enjoy it.
...
The original data set had 672 rows for 316 IDs and 21 variables...DO WE NEED MORE HERE?
...
Our dataset was then joined by the additional exam_data_joint.txt, by the common variable ID. By the end of the tidying session we ended up with 316 IDs( rows) and 27 variables (columns).
...
PENDING: You can just skip this part, i put this to convert the .txt to html and open it in the .rmd every time, but i think we need to format the codebook first. There is a Rmd version of the codebook now. The chunk below. It looks more like the original codebook, check it out. If not we can try to dispay it another way. I can see if we could make it the same way that the original codebook we got, if not, we just leave like that... (JP-Lørdag 20.09)
...
```{r}
knitr::opts_current$set(echo = FALSE, results = "asis")
...
After a fun session looking at how to create and format plots in R, we were able to create some nice figures (according to us) to try to answer the following questions. Before each figure, we included the script we used to build these plots according to each task (just for informative purposes).
