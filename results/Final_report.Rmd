...
We created our GitHub repository named "Blank_exam" to place our documents. There you can find the README file with the overview of our directory.
...
Further on we made new variables to filter and explore the dataset. For instance PVol_High as categorical variable ( \>/\< 100 ml) The variables unit_norm, ttr_num??? ??

Afterwards, when looking over our variables, we wanted to tidy the variable Hosp, deleting characters and keeping the number (1-3) of the treating hospital.
...
Our dataset was then left joined with the additional exam_data_joint.txt, by the common variable ID. By the end of the tidying session we ended up with 316 IDs( rows) and 27 variables (columns).
...
On this new joint dataset, we created a new variable "TimeToRecurrence_days_new", with the purpose of making the variable "TimeToRecurrence_weeks_new", for calculations with the same numeric category (weeks). We met a little rocky road making this new variable, therefore keeping some duplicates to remember the steps of the string.
...
Once we were done with these changes and joining our data sets, we began exploring our variables in detail, based on the following instructions (we include the commands we used for each task):
...
Overview of our last data set before working on making figures.
...
Additionally, we created a supplementary codebook based on the variables we created, changed and renamed.
...
After tidying several times, joining, more tyiding, stratifying data, showing several overviews of our different data sets, creating plots and analyzing the results, we have finally come to the end of our work report. We must admit this course and the work felt like a roller coaster, with fun and very productive days when all the commands, libraries and tables came through, and with days when when not even RStudio would open. We tried to do our best on this report and have fun while working on it. We hope is clear and complete enough for you to approve it. Thank you for reading it.

THE BLANK TEAM (that is not so blank anymore after this course).

```{r, echo=FALSE, fig.align='center', out.width="30%"}
knitr::include_graphics(
  here("other files", c("Anders.jpg", "JP.jpg", "Guri.JPG"))
)
```
